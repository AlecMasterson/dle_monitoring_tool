...
def create_app(database):
    ...
    def apiStatusMonitor():
        ...
        numDone = 0
        totalRuntime = 0
        serviceCounts = {service: {'count': 0, 'statuses': {
            status: 0 for status in ['ALL'] + statuses}} for service in ['ALL'] + services}

        finalDF = pandas.DataFrame()
        for ag in ags:
            agData = statusDF[statusDF['ag'] == ag]
            if len(agData) == 0:
                notStarted.append(ag)
                continue
            ...
            for service in services:
                ...
                serviceCounts['ALL']['count'] += 1
                serviceCounts['ALL']['statuses']['ALL'] += 1
                serviceCounts['ALL']['statuses'][status['status']] += 1

                serviceCounts[service]['count'] += 1
                serviceCounts[service]['statuses']['ALL'] += 1
                serviceCounts[service]['statuses'][status['status']] += 1
